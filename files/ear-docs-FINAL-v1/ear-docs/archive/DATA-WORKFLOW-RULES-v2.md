...
> **VERIFY (fake-gzip finding):** historical data contained files named `.jsonl.gz` that were **not actually compressed** (loaders forced `compression='uncompressed'`). Confirm the current agent really gzips; ingest must detect actual compression rather than trust the extension.
