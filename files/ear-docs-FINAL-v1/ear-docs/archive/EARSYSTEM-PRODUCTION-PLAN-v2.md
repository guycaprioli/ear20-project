...
### Transport decision (the biggest lever)

Your agents currently upload to Cloudflare R2, and a sync tool pulls it back down. **On one site that's a cloud round-trip for data that never needed to leave the building.**

- **Option A — LAN-primary (RECOMMENDED).** Agents write to a server-hosted share (NFS/SMB) **or** a local S3 (MinIO) on the server. The uploader's existing S3 code path points at MinIO instead of R2 by changing the endpoint — near-zero agent code change. R2 becomes an optional nightly off-site backup push. **Removes cloud egress, latency, and Cloudflare as an availability dependency for local reporting.**
- **Option B — R2-primary (keep current).** Simplest to leave as-is; agents already push to R2, server syncs down. Justified only if off-site access to raw data matters more than on-site independence, or if the warehouse LAN is unreliable.

**Recommendation: A, with MinIO.** It keeps the S3 semantics every tool already speaks (so `read_json`, the uploader, and any sync code work unchanged), keeps everything on-prem, and R2 stays available as `rclone`-style backup. If you'd rather not run MinIO, a plain NFS share + the existing directory-scan loader is even simpler.
...
Agents upload on a timer; a straggler PC can upload a backlog whose *filenames* fall outside a "last N hours" window. Filename-date filtering would silently skip never-ingested files. **Fix:** an `ingested_files` table in DuckDB (filename, size/etag, rows, ingested_at). Loader = list → anti-join ledger → load only new → record in the **same transaction** as the INSERT. R2/MinIO PUTs are atomic, so no half-file risk. Uploads are write-once (5-min/50 MB rotation, pending→processing→completed), so filename keying is safe; add size/etag if agents could ever re-push a grown file.
...
- **Point uploader at MinIO** (Option A): change S3 endpoint/creds; no logic change.
...
- **MinIO** service (Option A) or an NFS mount for `raw_data`.
...
**Phase 0 — Consolidate & decide (0.5 wk).** Land this plan in `bss_earv1` as `docs/`. Archive ear-compared, earsyncr2core (keep verify-sync if wanted). Confirm transport = Option A/MinIO or NFS.
...
**Phase 3 — Correlation + reporting (1–1.5 wk).** `Sensor` band-group correlation → `CorrelatedDetection`. Port `ForensicReportGenerator` as Celery task. Dashboard: run history, candidate tables, correlation view, report downloads (shadcn components already present).
...
- **Transport (Option A vs B).** _Your call._ Recommendation A/MinIO. Blocks Phase 5, not earlier phases.
...
- **Single-site assumption.** Plan assumes one LAN. Multi-site later → revisit transport (R2-primary per site + central server) and `Sensor.site`.
