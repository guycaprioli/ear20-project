...
| P15 | **Deep-sleep survival (end-to-end)** ⚠ | One device: 4 short bursts across a 10-day window (multi-day gaps), stable power, recent first-seen. | Survives noise filter (NOT dropped by min-obs); events built (min-events rules must not erase it); classified **G1 deep-sleep**, not noise/unclassified-by-starvation. **The primary target's hardest case — if this persona is red, the system misses 20595-class devices.** | Low-duty exception × min-obs × min-events interplay; taxonomy G1 |
...
- **Determinism:** run the full chain twice on the fixture → byte-identical `events` Parquet.
